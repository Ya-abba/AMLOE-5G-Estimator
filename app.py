...
import math
import folium
from folium.plugins import HeatMap  # Added for heatmap support
...
def load_resources():
    try:
        ...
        #model = joblib.load("rsrp_model.pkl")rsrp_model_compressed
        model = joblib.load("rsrp_model_compressed.pkl")
        return df, model
    except Exception as e:
        st.error(f"Initialization Error: {e}. Check if files exist in the folder.")
        return None, None
...
with col2:
    ...
    if show_heatmap and model is not None:
        with st.spinner("Calculating area coverage grid..."):
            # Create a 25x25 grid around the BS (approx 400m radius)
            grid_res = 25
            span = 0.004
            ...
            for g_lat in lat_grid:
                for g_lon in lon_grid:
                    ...
                    if dist < 5: continue  # Skip point exactly at BS
                    ...
                    # Predict RSRP: [D, AZoffset, ELEV, TILToffset, FQ, Placeholder]
                    X_grid = [[dist, az_off, elev, tilt_off, freq, 0]]
                    rsrp_pred = model.predict(X_grid)[0]

                    # Normalize weight for Folium HeatMap (0 to 1)
                    # -120dBm -> 0 | -70dBm -> 1
